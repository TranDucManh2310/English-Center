(function() {
  ...
  function renderHonorStudents() {
    ...
    if (!state.honorStudentIds.size) {
      const topIds = (state.dashboard && state.dashboard.topStudents || []).map(item => item.id).filter(Boolean);
      topIds.slice(0, 5).forEach(id => state.honorStudentIds.add(id));
      if (!state.honorStudentIds.size) state.students.slice(0, 5).forEach(item => state.honorStudentIds.add(item.id));
    }
